...
/**
 * Formats the rerank query for Cohere Rerank v4 Pro.
 *
 * Uses the compressed 1-sentence `cohereSemanticTarget` (generated by Gemini) if provided,
 * which eliminates secondary noise words and temporal bias for optimal cross-attention.
 * Falls back to combining `researchCore` and `targetActors` if not provided.
 *
 * @param matrix - The thesis matrix.
 * @param cohereSemanticTarget - Optional 1-sentence compressed semantic target string.
 * @returns Clean query string for Cohere Rerank.
 */
export function formatRerankQuery(
  matrix: ThesisMatrix,
  cohereSemanticTarget?: string,
): string {
  if (cohereSemanticTarget && cohereSemanticTarget.trim().length > 0) {
    return cohereSemanticTarget.trim();
  }
  return `${matrix.researchCore} - ${matrix.targetActors}`;
}
