...
export const PDF_PARSER_SYSTEM_INSTRUCTION = `# Rol ve Uzmanlık
...
1. **Grounded Extraction & Verbatim Preservation:**
   ...
   - Output null for missing metadata fields (publisher name, year, city, etc.) when absent from source text.
...
3. **Formal Bibliography Extraction:**
   ...
   - Parse each reference into explicit fields: raw (verbatim text without leading entry numbers), documentType, title, containerTitle, authors (with name and role: author/editor/translator), year (extracting outer edition year or 4-digit Gregorian year), publisher, and publisherPlace.
