...
# Birincil Görev
Yukarıda sağlanan tez matrisini inceleyin. Sistem talimatındaki evrensel alokasyon kurallarını, üçlü vektör sorgu formülünü ve kadran kurallarını uygulayarak 5 kadranlı epistemolojik kutu yapısını JSON formatında üretin. Alt kutular arasında kavram veya aktör çakışması olmamasına özen gösterin.

# DİL VE VEKTÖR PARAGRAF TALİMATI (ÇOK ÖNEMLİ)
1. JSON içerisindeki "title", "description" ve "concepts" alanlarının TAMAMI İSTİSNASIZ AKADEMİK TÜRKÇE OLMALIDIR. Sadece ve sadece subBoxes içerisindeki "semanticQuery" alanı İngilizce akademik arama metni olarak üretilecektir (PRIMARY_MATERIAL kadranında semanticQuery her zaman boş string '' olmalıdır).
2. "semanticQuery" ALANLARI KISA KELİME LİSTELERİ VEYA TAG'LER OLMAMALIDIR! OpenAlex AI Vektör Arama Motoru (GTE Large EN modeli) için şu 3'lü formüle dayalı 2-4 cümlelik (300-800 karakterlik) zengin, akademik arama paragrafları üretin:
   \`semanticQuery = [Core Empirical/Theoretical Objective] + [Specific Entity & Context Anchors] + [High-Yield Literature Domain Keywords & Canonical Terms]\``;
}
