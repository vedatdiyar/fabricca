...
- ONTOLOGICAL FILTER RULES:
  ...
  3. CONTEXT — Macro-Historical Rupture on the Core Subject: Synthesize the DIRECT CAUSAL AND STRUCTURAL PRESSURE of the input's macro historical/geopolitical ruptures (e.g., the post-Cold War transition, collapse of political current Z) and structural factors ON the Core Subject, anchored specifically to the 'spatialScope' and 'temporalScope' provided in the input matrix. The query must lock onto this regional and temporal setting to avoid generic world history or unrelated international relations literature.
