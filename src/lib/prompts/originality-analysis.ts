...
export const PARAM_DEFINITIONS: ParamDefinition[] = [
  {
    ...
    description:
      "The degree to which the candidate thesis's primary research focus — including both the core intellectual problem AND the main actors, subjects, or analytical objects — matches the target's research core. Evaluate the COMBINED research problem and its subjects: what specific phenomenon, relationship, process, or knowledge gap is investigated, AND which actors/groups/organizations are the primary subjects of analysis. Score 100 if the candidate investigates the same or nearly identical problem-actor constellation. Score 50 if the candidate investigates a related but distinct sub-problem within the same domain, OR if the same problem is studied through different but adjacent actors/subjects. Score 0 if the candidate's primary inquiry falls into a fundamentally different domain — even if isolated contextual elements appear similar.",
    isStringEnum: false,
    jsonSchema: {
      type: "array",
      items: {
