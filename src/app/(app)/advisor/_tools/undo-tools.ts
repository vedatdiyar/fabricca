...
function buildBoxUpdateData(
  previousState: Record<string, unknown>,
): { title?: string; description?: string | null; updatedAt: Date } {
  const updateData: {
    title?: string;
    description?: string | null;
    updatedAt: Date;
  } = { updatedAt: new Date() };
  if (typeof previousState.title === "string") updateData.title = previousState.title;
  if (
...
function buildSourceUpdateData(
  ...
  if (typeof previousState.title === "string") updateData.title = previousState.title;
  if (typeof previousState.isRead === "boolean") updateData.isRead = previousState.isRead;
  return updateData;
}
...
async function undoUpdateThesisMatrix({
  ...
  await db.update(matrices).set(updateData).where(eq(matrices.id, userMatrix.id));

  const execData = executionResult as
    | { cascade?: { createdBoxes?: { id: number }[] } }
    | undefined;
...
async function undoCreateBox({ args, executionResult }: UndoParams): Promise<UndoResult> {
  const createdBox = executionResult as Box | undefined;
  const boxId = createdBox?.id ?? (args.boxId as number | undefined);
  if (!boxId) return { success: false, error: "Silinecek kutu kimliği bulunamadı." };
  await db.delete(boxes).where(eq(boxes.id, boxId));
  return { success: true, message: "Oluşturulan kutu geri alındı (silindi)." };
}

async function undoUpdateBox({ args, previousState }: UndoParams): Promise<UndoResult> {
  const boxId = args.boxId as number;
  if (!previousState) return { success: false, error: "Önceki kutu verisi bulunamadı." };
...
async function undoDeleteBox({ previousState }: UndoParams): Promise<UndoResult> {
  if (!previousState) return { success: false, error: "Silinen kutu yedeği bulunamadı." };
...
async function undoCreateOutlineSection({
  ...
  const outlineId = createdOutline?.id ?? (args.outlineId as number | undefined);
  if (!outlineId) return { success: false, error: "Silinecek bölüm kimliği bulunamadı." };
  await db.delete(outlines).where(eq(outlines.id, outlineId));
  return { success: true, message: "Oluşturulan bölüm planı geri alındı (silindi)." };
}
...
async function undoUpdateOutlineSection({
  ...
  if (!previousState) return { success: false, error: "Önceki bölüm verisi bulunamadı." };
...
async function undoAddNote({ args, executionResult, userId }: UndoParams): Promise<UndoResult> {
  const createdNote = executionResult as Annotation | undefined;
  const noteId = createdNote?.id ?? (args.noteId as number | undefined);
  if (!noteId || !userId) return { success: false, error: "Silinecek not kimliği bulunamadı." };
  await db.delete(annotations).where(and(eq(annotations.id, noteId), eq(annotations.userId, userId)));
  return { success: true, message: "Eklenecek not geri alındı (silindi)." };
}

async function undoDeleteNote({ previousState, userId }: UndoParams): Promise<UndoResult> {
  if (!previousState || !userId) return { success: false, error: "Silinen not yedeği bulunamadı." };
...
async function undoPinAnnotationToOutline({ args }: UndoParams): Promise<UndoResult> {
  const outlineId = args.outlineId as number;
  const annotationId = args.annotationId as number;
  if (!outlineId || !annotationId) return { success: false, error: "Geçersiz bölüm veya alıntı kimliği." };
  await db.delete(outlineAnnotations).where(and(eq(outlineAnnotations.outlineId, outlineId), eq(outlineAnnotations.annotationId, annotationId)));
  return { success: true, message: "Alıntı iğnelemesi geri alındı." };
}

async function undoUnpinAnnotationFromOutline({ args }: UndoParams): Promise<UndoResult> {
  const outlineId = args.outlineId as number;
  const annotationId = args.annotationId as number;
  if (!outlineId || !annotationId) return { success: false, error: "Geçersiz bölüm veya alıntı kimliği." };
...
async function undoUpdateSource({ args, previousState }: UndoParams): Promise<UndoResult> {
  const sourceId = args.sourceId as number;
  if (!previousState) return { success: false, error: "Önceki kaynak verisi bulunamadı." };
...
async function undoDeleteSource({ previousState }: UndoParams): Promise<UndoResult> {
  if (!previousState) return { success: false, error: "Silinen kaynak yedeği bulunamadı." };
  await db.insert(sources).values({
    boxId: previousState.boxId as number,
    title: previousState.title as string,
    authors: (previousState.authors as string[] | null) ?? undefined,
    publicationYear: (previousState.publicationYear as number | null) ?? undefined,
...
async function undoLinkSourceToOutline({ args }: UndoParams): Promise<UndoResult> {
  const outlineId = args.outlineId as number;
  const sourceId = args.sourceId as number;
  if (!outlineId || !sourceId) return { success: false, error: "Geçersiz bölüm veya kaynak kimliği." };
  await db.delete(outlineSources).where(and(eq(outlineSources.outlineId, outlineId), eq(outlineSources.sourceId, sourceId)));
  return { success: true, message: "Kaynak bağlantısı geri alındı." };
}

async function undoUnlinkSourceFromOutline({ args }: UndoParams): Promise<UndoResult> {
  const outlineId = args.outlineId as number;
  const sourceId = args.sourceId as number;
  if (!outlineId || !sourceId) return { success: false, error: "Geçersiz bölüm veya kaynak kimliği." };
...
async function undoCreateTask({ args, executionResult, userId }: UndoParams): Promise<UndoResult> {
  const createdTask = executionResult as Task | undefined;
  const taskId = createdTask?.id ?? (args.taskId as number | undefined);
  if (!taskId || !userId) return { success: false, error: "Silinecek görev kimliği bulunamadı." };
  await db.delete(tasks).where(and(eq(tasks.id, taskId), eq(tasks.userId, userId)));
  return { success: true, message: "Oluşturulan görev geri alındı (silindi)." };
}

async function undoUpdateTaskStatus({ args, previousState, userId }: UndoParams): Promise<UndoResult> {
  const taskId = args.taskId as number;
  if (!previousState || !userId) return { success: false, error: "Önceki görev durumu bulunamadı." };
...
export async function undoMutationTool(
  ...
  const domainHandlers: Array<(name: string, p: UndoParams) => Promise<UndoResult | null>> = [
    undoBoxMutation,
    undoOutlineMutation,
  ...
  return { success: false, error: "İşlem geri alınamadı. Lütfen tekrar deneyin." };
}
