...
- **Tekil ve Bütünleşik Metodolojik Hat (N=1):** Matriste yalnızca tek bir araştırma ve analiz yöntemi (örneğin salt söylem analizi, salt arşiv incelemesi veya salt vaka analizi) ve onun kendi içsel analitik adımları, açık uçlu soruları, korpus sınırlama kriterleri veya dönemselleştirmesi (kritik dönemler, kırılma noktaları) yer alıyorsa KESİNLİKLE TEK BİR ALT KUTU (N=1) kullanılır. Yöntemin iç adımları, analitik soruları, örneklem sınırlaması veya dönemleme evreleri yapay olarak ayrı metodoloji kutularına bölünemez.
