 ...
 * source links.
 */
export function TavilyFactCheckTable({ tavilyResults }: TavilyFactCheckTableProps) {
  return (
    <Card>
      <CardHeader>
                    ...
                    <td className="p-3 text-left max-w-[320px]">
                      {isKnownEnum ? (
                        <div className="space-y-2">
                          ...
                          {item.resultNote && (
                            <p className="text-muted-foreground font-light text-xs leading-relaxed">{item.resultNote}</p>
                          )}
                        </div>
                      ) : (
