  ...
  return (
    <div className="flex flex-1 flex-col bg-background text-foreground p-6 md:p-10 pb-24 md:pb-10 overflow-y-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8 flex-1">
        {/* MOBILE VIEW: Horizontal scrollable cards for Thesis Core parameters */}
        <div className="lg:hidden block space-y-2">
        ...
        {/* DESKTOP VIEW: Left Column (Thesis Constitution Card) */}
        {thesisData && (
          <div className="hidden lg:block lg:col-span-1 border border-border bg-card p-6 rounded-lg space-y-6 shadow-xl h-fit relative overflow-hidden">
            <div className="absolute top-0 left-0 w-1 h-full bg-primary" />
            ...
            <div className="space-y-5">
              <div className="space-y-1.5">
                ...
                <p className="text-xs text-muted-foreground leading-relaxed bg-background p-3.5 rounded border border-border">
                  {thesisData.researchQuestion}
                </p>
              </div>
        ...
        {/* Right Column: Dynamic Planner and Literature Recommendations */}
        <div className="lg:col-span-3 space-y-8">
          {/* Active Tasks list */}
          <div className="border border-border bg-card p-6 rounded-lg shadow-xl space-y-6">
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 pb-4 border-b border-border">
              ...
              <button
                ...
                className="text-xs font-semibold bg-accent text-accent-foreground border border-border px-3.5 py-2 rounded hover:bg-background transition cursor-pointer flex items-center gap-1.5 self-start sm:self-auto"
              >
              ...
              {activeTasks.length === 0 ? (
                ...
                activeTasks.map((task) => (
                  ...
                  >
                    ...
                    <div className="flex items-center justify-end border-t border-border sm:border-0 pt-2 sm:pt-0 gap-2 shrink-0">
            ...
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 pb-4 border-b border-border">
              <div>
                <h2 className="text-base font-bold text-foreground flex items-center gap-2">
                  ...
                  <span>Haftalık Yapay Zeka Kaynak ve Literatür Önerileri</span>
                </h2>
                <p className="text-xs text-muted-foreground font-sans mt-0.5">
                  Tez Anayasanızdaki argüman ve araştırma sorusuna derinlik
                  katacak yapay zeka tarafından önerilen temel akademik kaynaklar
                </p>
              </div>
              {thesisData && (
                <button
                  ...
                  className="text-xs font-semibold border border-primary text-primary bg-background hover:bg-primary hover:text-primary-foreground transition duration-150 rounded px-3.5 py-2 cursor-pointer flex items-center gap-1.5 self-start sm:self-auto disabled:opacity-50 shrink-0"
                >
                  <Sparkles className="size-3.5" />
                  <span>{isLoadingRecs ? "Taranıyor..." : "Literatürü Genişlet"}</span>
                </button>
              )}
            </div>
                  ...
                  >
                    <div className="space-y-3">
                      <div className="flex justify-between items-start gap-2">
                        ...
                        {typeof rec.citationCount === "number" && rec.citationCount > 0 && (
          ...
          {completedTasks.length > 0 && (
            <div className="border border-border bg-card p-6 rounded-lg shadow-xl space-y-4">
              ...
              <div className="space-y-2.5">
                {completedTasks.slice(0, 3).map((task) => (
                  <div
                    key={task.id}
                    className="bg-background/50 border border-border p-3.5 rounded-lg flex items-center justify-between gap-4"
                  >
                    <div className="flex-1">
                      <p className="text-xs font-sans text-muted-foreground line-clamp-1 line-through">
                        {task.taskDescription}
                      </p>
                    </div>
                    ...
                    <button
                      ...
                      className="text-[9px] font-bold text-primary bg-accent hover:bg-background border border-border px-2 py-1 rounded transition duration-150 cursor-pointer"
                    >
                      Geri Al
                    </button>
                  </div>
