...
Tavily (web) ve TEZARA (YÖK Ulusal Tez Merkezi) API'leri üzerinden eşzamanlı
çalışan çift kademeli bir analiz motorudur:

- **Web taraması:** Çalışmanızın temel iddialarının internetteki mevcut
  literatürle örtüşme/çelişme durumunu maddi olarak denetler.
...
| **State Yönetimi**     | Zustand (sessionStorage)                              |
...
│   ├── tavily.ts                    # Tavily web arama API
...
| `originality_reports` | Özgünlük raporları  | userId (unique), tavilyResults (JSONB), tezaraResults (JSONB)                |
...
1. Tez Matrisi (Matrix) — 6 temel alanın tanımı
         ↓
2. (Enrichment — kaldırıldı)
         ↓
3. Özgünlük & Risk Analizi (Risk)
   ├─ Tavily ile web taraması (fact-checking)
...
Platform, dışarıdan kayda kapalı olduğu için veri tabanına seed edilmiş iki
kullanıcı ile gelir:

| İsim                   | E-posta                        |
| ---------------------- | ------------------------------ |
| Vedat Diyar Çelikkeser | vedatdiyarcelikkeser@gmail.com |
| Tuğba Ahncr            | tubaahncr@gmail.com            |
