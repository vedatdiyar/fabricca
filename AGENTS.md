# Fabricca — Mühendislik Anayasası (AGENTS.md)

> **Son Güncelleme:** 31 Ağustos 2026  
> **Statü:** Yürürlükte  
> **Kapsam:** Repository genelinde geçerli mühendislik, mimari ve süreç politikası

Bu doküman, Fabricca repository’si için kalıcı mühendislik kurallarını tanımlar. Tüm katkılar bu çerçeveye tabidir.

---

## 1. Rol ve Sistem Özeti (Role & System Overview)
...
## 2. Temel Teknoloji Yığını ve Mimari Kısıtlar (Core Stack & Architectural Constraints)

- **Framework:** Next.js (App Router, Server Actions)
- **Database & ORM:** Neon Serverless PostgreSQL (`pgvector`), Drizzle ORM
- **Primary LLM:** Google Gemini Flash (`FLASH_LITE_35`, `FLASH_38` via `@google/genai`; fallbacks `FLASH_LITE_35 → FLASH_LITE_31`, `FLASH_38 → FLASH_37`)
- **Embedding Motoru:** Cloudflare Workers AI (`@cf/baai/bge-m3` — SINGLE source RAG embedding model; thesis-search uses `intfloat/multilingual-e5-base` (768d) as the only documented exception, see `docs/ARCHITECTURE.md:2`)
...
## 3. Altın Sınır Kuralı (The Golden Boundary Rule)
...
## 4. Kod ve Doğrulama Kuralları (Code & Verification Rules)
...
## 5. Göreve Özgü Dokümantasyon (Task-Specific Documentation — Progressive Disclosure)
...
## 6. Talimat Önceliği ve Bakım (Instruction Precedence & Maintenance)
