...
- **PDF Parçalama ve Metadata Çıkarımı:** `@opendocsg/pdf2md` (yerel ultra hızlı <200ms Markdown dönüştürme) + `pdf-lib` (LlamaParse batch'leri için istenen sayfa aralıklarının kesildiği PDF page slicing — `src/lib/services/pdf/page-slicer.ts`) + `normalizeTurkishText` (Unicode Türkçe diyakritik onarımı) + LlamaParse API (taranmış/görsel PDF'ler için OCR çözücü; geçici ağ hataları/429/5xx için `@/lib/api-utils` `withRetry` ile exponential backoff), Crossref API (DOI ile metadata çözümleme), Gemini Flash-Lite (DOI bulunamadığında metin tabanlı metadata çıkarımı). Taranmış veya karmaşık düzene sahip PDF'lerde "llamaparse-fallback" rotası doğrudan LlamaParse servislerine yönlendirilir.
