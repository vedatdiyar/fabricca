...
- **AI Orkestrasyonu:** Vanilla TypeScript (LangChain vb. ağır kütüphaneler kaldırılmıştır)
...
- **PDF'ten Markdown'a Dönüştürücü:** Resmi **LlamaParse API** (Aylık 7.000 sayfa ücretsiz kota, sayfa düzeni ve çift sütun farkındalıklı parser)
...
- **Masaüstü:** Sol tarafta tezin yönünü kaybetmemesi için "Tez Anayasası" özet kartı sabit durur. Sağ tarafta ise Gemini ve Semantic Scholar API'nin ortaklaşa ürettiği haftalık 3 makale önerisini içeren modern bir Kanban veya yapılacaklar listesi (`Tasks`) yer alır.
...
- **Masaüstü:** Ekranın sol tarafında hızlı referans için "Kütüphanedeki PDF'ler ve Notlar" listesi (kullanıcının o anki tartışmaya hangi kaynakları dahil etmek istediğini seçebileceği check-box yapısı ile), sağ tarafında ise Gemini RAG mimarisiyle beslenen geniş bir akademik chat ekranı.
...
Uygulamanın arka planında çalışan tüm yapay zeka ve veri işleme mantığı, Gemini API ve vanilla TypeScript RAG mimarisi üzerinden yürütülecektir. Kodlama ajanının bu akışları kurarken uygulaması gereken algoritmik adımlar ve entegrasyon kuralları şunlardır:
...
### B. PDF to Markdown ve Vanilla TS Akıllı Bölme (Chunking) Akışı
...
2. Next.js backend, resmi **LlamaParse SDK**'sını tetikler. PDF, LlamaParse API'sine gönderilir.
...
4. **Anlam Bütünlüğünü Koruyan Bölme Protokolü:** Özel Markdown başlık ayırıcısı (`splitMarkdownByHeaders`) fonksiyonu ile metin önce yapısal başlıklarına (`#`, `##`, `###`) göre odalara ayrılır. Hemen ardından `splitTextToChunks` (vanilla TS) fonksiyonumuz tetiklenerek her oda kendi içinde `chunkSize: 1000` karakter ve `chunkOverlap: 200` karakter (örtüşme penceresi) parametreleriyle bölünür. Böylece bir argümanın ortadan ikiye kesilmesi engellenir ve her parça bir önceki/sonraki bağlamdan izler taşır.
...
### 🟧 FAZ 3: Vanilla TS + LlamaParse + Embedding RAG Boru Hattı

- **Gereksinimler:**
- Vanilla TypeScript metin bölücülerin kurulması.
- Kullanıcı PDF yüklediği an tetiklenecek bir Next.js Server Action / API rotası yazılması: Dosyayı LlamaParse'a gönder, dönen temiz Markdown metnini custom `splitMarkdownByHeaders` ve `splitTextToChunks` (1000 size, 200 overlap) süzgecinden geçir.
- Oluşan parçaları `gemini-embedding-2` modeli ile vektörleştirip Neon PostgreSQL `pdf_chunks` tablosuna başarılı şekilde kaydet.
...
| 🟧 **FAZ 3** | Vanilla TS + LlamaParse + Embedding RAG Boru Hattı | ✅ **TAMAMLANDI** | LlamaParse v2 entegrasyonu, custom splitMarkdownByHeaders + splitTextToChunks (1000/200), Gemini embedding-2 ile 1536D vektör çıktısı, pdf_chunks tablosuna kayıt |
