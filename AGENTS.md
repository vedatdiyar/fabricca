...
- Onboarding Veri Tabanı Kayıt Kuralları (Toplu Kayıt & Sıfırlama Kalkanı):
  - **Database-Free Mimari:** Onboarding süresince (Matrix, Zenginleştirme, Risk Analizi ve Konu Kutusu adımlarında) üretilen zenginleştirilmiş tez verileri, jüri raporları, taranan/elenen tezler ve konu kutuları veritabanına yazılmaz. Tamamı server action'lar tarafından saf JSON olarak frontend'e döner.
  ...
  - **Nihai Toplu Kayıt Transaction Mimarisi (Faz 4):** Kullanıcı en son adımda "Tamamla" butonuna bastığı an, Zustand store'daki tüm veriler (`formData`, `approvedKeywords`, `juryReport`, `boxes`) tek bir server action (`confirmBoxesAction`) parametresi olarak gönderilir ve Drizzle `db.transaction` bloğu içinde atomik olarak kaydedilir:
    - 1. `thesis_matrices` tablosuna nihai veriyi ve onaylı anahtar kelimeleri (`keywords` kolonu) yazar.
    - 2. `originality_reports` tablosuna jüri raporunu (`tavilyResults` ve `tezaraResults` JSONB alanlarına) yazar.
    - 3. `thesis_boxes` tablosuna hiyerarşik konu kutularını (ebeveyn kategoriler ve bunlara bağlı alt sub-box'lar) yazar.
    - 4. `users` tablosundaki kullanıcının `onboarding_completed` değerini `true` yapar.
  - **Oturum (Session Cookie) Kilitleme:** Veritabanı transaction'ı başarıyla tamamlandığında, `fabricca_session` cookie'si sunucu tarafında anında güncellenerek `onboardingCompleted: true` olarak mühürlenir. Böylece sayfa yenilenmesine gerek kalmadan router veya düzen (layout) guard'ları anında tetiklenir ve dashboard geçişi açılır.
  ...
  - **Literatür Tarama (Literature Review) Gelecek Planı:** Kutular (Boxes) adımından hemen sonra eklenecek olan "Literatür Tarama" adımı bu sisteme entegre edilecektir. Final toplu transaction yapısı ve yönlendirmeleri, araya bu yeni adımın kolayca girebileceği şekilde modüler ve esnek tutulmuştur. Final kayıt aksiyonuna yeni parametreler veya ara veri yapıları eklenebilir.
