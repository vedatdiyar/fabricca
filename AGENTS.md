...
- **Dinamik Veri Tabanı ve Otomatik Senkronizasyon (Push):** Veri tabanı şeması (`db/schema.ts`) projenin ihtiyaçlarına göre dinamik olarak değişebilir. Ajan, şemada herhangi bir tablo, kolon veya ilişki değişikliği yaptığı an, kod tabanını bırakıp gitmeden önce **mutlaka** `npm run db:push` komutunu çalıştırarak veri tabanını kodla %100 senkronize hale getirmekle yükümlüdür.
